## Add data for display
 ...
 # library(stringr)
 # 
 # df_loupan = readxl::read_xlsx("./data/hefei_xinloupan_2020-08-02.xlsx") %>% select(-c(1)) %>% distinct()
 ...
 # df_ershou = readxl::read_xlsx("data/hefei_ershoufang_2020-07-31.xlsx") %>%
 #         distinct()
 # df_ershou = df_ershou %>% mutate(Total_Price = str_replace(Total_Price,"\\s","")) %>%
 #   mutate(price = str_extract(Unit_Price, "\\d+"))
 # 
 # df_ershou_stat = readxl::read_xlsx("data/hefei_ershoufang_num_stat.xlsx")
